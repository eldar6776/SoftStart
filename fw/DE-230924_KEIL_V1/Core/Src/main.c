...
#define TRAFO_VOLTAGE_TRESHOLD      1000    // adc value for 24Vac trafo valid output 
...
#define SS_SET_TRAFO_VTRESHOLD      0xA5
/* USER CODE END PD */
...
#define TrafoVoltageMonitorUsed()       (sys_flags |= (1U << 2))
#define TrafoVoltageMonitorNotUsed()    (sys_flags &= (~(1U << 2)))
#define IsTrafoVoltageMonitorUsed()     (sys_flags & (1U << 2))
#define TrafoTemperatureSensorUsed()    (sys_flags |= (1U << 3))
#define TrafoTemperatureSensorNotUsed() (sys_flags &= (~(1U << 3)))
#define IsTrafoTemperatureSensorUsed()  (sys_flags & (1U << 3))
#define TriacTemperatureSensorUsed()    (sys_flags |= (1U << 4))
#define TriacTemperatureSensorNotUsed() (sys_flags &= (~(1U << 4)))
#define IsTriacTemperatureSensorUsed()  (sys_flags & (1U << 4))
#define TrafoVoltageErrorSet()          (sys_flags |= (1U << 5))
#define TrafoVoltageErrorReset()        (sys_flags &= (~(1U << 5)))
#define IsTrafoVoltageErrorActiv()      (sys_flags & (1U << 5))
#define TrafoTemperatureErrorSet()      (sys_flags |= (1U << 6))
#define TrafoTemperatureErrorReset()    (sys_flags &= (~(1U << 6)))
#define IsTrafoTemperatureErrorActiv()  (sys_flags & (1U << 6))
#define TriacTemperatureErrorSet()      (sys_flags |= (1U << 7))
#define TriacTemperatureErrorReset()    (sys_flags &= (~(1U << 7)))
#define IsTriacTemperatureErrorActiv()  (sys_flags & (1U << 7))
/* USER CODE END PM */
...
uint16_t trafo_temp;
uint16_t triac_temp;
uint16_t sec_voltage = TRAFO_VOLTAGE_TRESHOLD;
	...
	while(1)
	{
		...
		switch(sys_state)
		{
			case INIT:
				HAL_Delay(500);
				sel = 0;
				if(HAL_GPIO_ReadPin(ADDRESS_0_GPIO_Port, ADDRESS_0_Pin) == GPIO_PIN_RESET) sel |= 0x01U;
				if(HAL_GPIO_ReadPin(ADDRESS_1_GPIO_Port, ADDRESS_1_Pin) == GPIO_PIN_RESET) sel |= 0x02U;
//				if(HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_14) == GPIO_PIN_RESET) sel |= 0x04U;
//				if(HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_13) == GPIO_PIN_RESET) sel |= 0x08U;
				...
				start_tout = sel * 3000U;
				HAL_Delay(500);
                SoftStartReady();
				TrafoVoltageErrorReset();
...
static void MX_GPIO_Init(void)
{
	...
	HAL_GPIO_WritePin(GPIOA, RELAY_CTRL_Pin | TRIAC_CTRL_Pin | STATUS_OUT_Pin, GPIO_PIN_RESET);
	...
	/*Configure GPIO pins : RELAY_CTRL_Pin TRIAC_CTRL_Pin STATUS_OUT_Pin */
	GPIO_InitStruct.Pin = RELAY_CTRL_Pin | TRIAC_CTRL_Pin | STATUS_OUT_Pin;
...
TF_Result STATUS_Listener(TinyFrame *tf, TF_Msg *msg)
{
	...
	if(sel == msg->data[0])
	{
		...
		ret[1] = sys_state;
		ret[2] = (trafo_volt >> 8);
		ret[3] = (trafo_volt & 0xFF);
		ret[4] = (trafo_temp >> 8);
		ret[5] = (trafo_temp & 0xFF);
		ret[6] = (triac_temp >> 8);
		ret[7] = (triac_temp & 0xFF);
		msg->data = ret;
		msg->len = 8;
		TF_Respond(tf, msg);
	}
	...
	if(sel == msg->data[0])
	{
		switch(msg->data[1])
		{
			...
			case SS_STOP_ALL:
				remote = false;
				break;
			case SS_SET_TRAFO_VTRESHOLD:
				sec_voltage = ((msg->data[1] << 8) + msg->data[2]);
				++res;
				break;
			default:
				break;
...
static void ADC3_Read(void)
{
	...
	if((HAL_GetTick() - adc_timer) >= ADC_READOUT_PERIOD)
	{
		...
		{
			trafo_volt = HAL_ADC_GetValue(&hadc);
			if(trafo_volt < SENSOR_VALID_DN_TRESHOLD)
			{
				TrafoVoltageMonitorNotUsed();
				TrafoVoltageErrorReset();
				trsec = true;
			}
			else if(trafo_volt > SENSOR_VALID_UP_TRESHOLD)
			{
				TrafoVoltageMonitorUsed();
				TrafoVoltageErrorSet();
				trsec = false;
				error = true;
			}
			...
			{
				TrafoVoltageMonitorUsed();
				if(trafo_volt >= sec_voltage) trsec = true;
				else trsec = false;
			}
			adc_cnt = 0;
		}
...
static void Error_Signal(void)
{
	...
	{
		if(IsTrafoVoltageErrorActiv() && (error_cnt == 0))
		{
			if(led_cnt == 0)
			{
				++led_cnt;
		...
		else if(IsTrafoTemperatureErrorActiv() && (error_cnt == 1))
		{
			if(led_cnt == 0)
			{
				++led_cnt;
			...
			else
			{
				if(led_rep)
				{
					--led_rep;
					led_tmr = HAL_GetTick();
					HAL_GPIO_TogglePin(STATUS_OUT_GPIO_Port, STATUS_OUT_Pin);
				}
				else
				{
					led_cnt = 0;
		...
		else if(IsTriacTemperatureErrorActiv() && (error_cnt == 2))
		{
			if(led_cnt == 0)
			{
				++led_cnt;
			...
			else if((HAL_GetTick() - led_tmr) >= led_tout)
			{
				if(led_rep)
				{
					--led_rep;
